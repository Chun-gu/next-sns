...
// TODO: 아래처럼 쓰기 편한 route를 어떻게 구현할까?
// route.posts // /posts
// route.posts(postId) // /posts/948
// route.posts(postId).edit // /posts/948/edit
...
// 2. 경로를 정의한 route 객체를 기반으로
// 자신을 반환하는 재귀적 클래스?
// export class Route {
// 	private static pathname = "";

// 	static posts(id?: string) {
// 		this.pathname += id ?? "";
// 		return this;
// 	}
// }

// const pathname = Route.posts();
...
class RouteBuilder {
	...
	private subRoutes: { [key: string]: RouteBuilder };

	constructor(
		segments: string[] = [],
		subRoutes: { [key: string]: RouteBuilder } = {},
	) {
		this.segments = segments;
		this.subRoutes = subRoutes;

		for (const key of Object.keys(this.subRoutes)) {
			Object.defineProperty(this, key, {
				get: () => this.subRoutes[key],
			});
		}
	}
...
class Route {
	...
	constructor(subRoutes: { [key: string]: RouteBuilder }) {
		this.subRoutes = subRoutes;

		// Dynamically generate methods for subRoutes
	...
	static async createFromFileSystem(dir: string): Promise<Route> {
		const subRoutes = await Route.readDirectory(dir);
		return new Route(subRoutes);
	}

	static async readDirectory(
		dir: string,
	): Promise<{ [key: string]: RouteBuilder }> {
		const entries = await fs.readdir(dir, { withFileTypes: true });
		const subRoutes: { [key: string]: RouteBuilder } = {};

		for (const entry of entries) {
			if (entry.isDirectory()) {
				const subRoute = await Route.readDirectory(path.join(dir, entry.name));
				subRoutes[entry.name] = new RouteBuilder([entry.name], subRoute);
			} else if (entry.isFile() && entry.name === "page.tsx") {
				subRoutes["index"] = new RouteBuilder();
			}
		}
...
console.log(paths);
// const route = await Route.createFromFileSystem('./app');
// console.log(route.posts.path());                          // "/posts"
// console.log(route.posts('3948').path());                  // "/posts/3948"
// console.log(route.posts('3948').edit.path());             // "/posts/3948/edit"
// console.log(route.users.path());                          // "/users"
// console.log(route.users('3948').path());                  // "/users/3948"
// console.log(route.users('3948').edit.path());             // "/users/3948/edit"
